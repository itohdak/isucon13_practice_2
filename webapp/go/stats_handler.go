...
func getUserStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(
		ctx,
		&reactionsList,
		"SELECT u.id AS id, COUNT(*) FROM users u INNER JOIN livestreams l ON l.user_id = u.id INNER JOIN reactions r ON r.livestream_id = l.id GROUP BY u.id",
	); err != nil {
	...
	if err := tx.SelectContext(
		ctx,
		&totalTipsList,
		"SELECT u.id AS id, IFNULL(SUM(l2.tip), 0) AS score FROM users u INNER JOIN livestreams l ON l.user_id = u.id INNER JOIN reactions r ON r.livestream_id = l.id GROUP BY u.id",
	); err != nil {
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(
		ctx,
		&reactionsList,
		"SELECT l.id AS id, COUNT(*) AS score FROM livestreams l INNER JOIN reactions r ON l.id = r.livestream_id GROUP BY l.id",
	); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count reactions: "+err.Error())
	}
	...
	if err := tx.SelectContext(
		ctx,
		&totalTipsList,
		"SELECT l.id AS id, IFNULL(SUM(l2.tip), 0) AS score FROM livestreams l INNER JOIN livecomments l2 ON l.id = l2.livestream_id GROUP BY l.id",
	); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count tips: "+err.Error())
	}
