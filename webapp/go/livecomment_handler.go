...
func postLivecommentHandler(c echo.Context) error {
	...
	if err := tx.GetContext(
		ctx,
		&count,
		"SELECT COUNT(1) FROM ng_words WHERE user_id = ? AND livestream_id = ? AND ? LIKE CONCAT('%', word, '%') LIMIT 1",
...
func moderateHandler(c echo.Context) error {
	...
	if _, err := tx.ExecContext(
		ctx,
		`DELETE FROM livecomments WHERE livestream_id = ? AND comment LIKE CONCAT('%', ?, '%')`,
		livestreamID,
		req.NGWord,
	); err != nil {
...
func fillLivecommentResponse(ctx context.Context, tx *sqlx.Tx, livecommentModel LivecommentModel) (Livecomment, error) {
	...
	commentOwnerModel, err := getUserByID(livecommentModel.UserID, tx, ctx)
	if err != nil {
	...
	livestreamModel, err = getLivestream(ctx, tx, livecommentModel.LivestreamID)
	if err != nil {
		return Livecomment{}, err
	}
...
func fillLivecommentReportResponse(ctx context.Context, tx *sqlx.Tx, reportModel LivecommentReportModel) (LivecommentReport, error) {
	...
	reporterModel, err := getUserByID(reportModel.UserID, tx, ctx)
	if err != nil {
